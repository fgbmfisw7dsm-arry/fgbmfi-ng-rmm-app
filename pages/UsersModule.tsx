                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        >
                            ...
                            <option value={UserRole.EXECUTIVE_ADMIN}>Executive Admin</option>
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
                                <div className="min-w-0 flex-1 w-full">
                                    <div className="flex items-center gap-3 mb-1 flex-wrap">
                                        ...
                                        <span className={`px-2.5 py-1 rounded-lg font-black uppercase text-[8px] tracking-widest shadow-sm ${u.role === UserRole.NATIONAL_ADMIN ? 'bg-blue-900 text-white' : u.role === UserRole.EXECUTIVE_ADMIN ? 'bg-indigo-800 text-white' : u.role === UserRole.REGIONAL_ADMIN ? 'bg-blue-700 text-white' : u.role === UserRole.DISTRICT_ADMIN ? 'bg-blue-500 text-white' : u.role === 'admin' ? 'bg-blue-900 text-white' : u.role === 'finance' ? 'bg-purple-600 text-white' : u.role === UserRole.EVENT_ADMIN ? 'bg-emerald-600 text-white' : 'bg-slate-500 text-white'}`}>
                                            {u.role === UserRole.NATIONAL_ADMIN ? 'Nat. Admin' : u.role === UserRole.EXECUTIVE_ADMIN ? 'Exec. Admin' : u.role === UserRole.REGIONAL_ADMIN ? 'Reg. Admin' : u.role === UserRole.DISTRICT_ADMIN ? 'Dist. Admin' : u.role === UserRole.NATIONAL_REGISTRAR ? 'Nat. Reg' : u.role === UserRole.REGIONAL_REGISTRAR ? 'Reg. Reg' : u.role === UserRole.DISTRICT_REGISTRAR ? 'Dist. Reg' : u.role === UserRole.ADMIN ? 'Sys Admin' : u.role === UserRole.REGISTRAR ? 'Registrar' : u.role === UserRole.EVENT_ADMIN ? 'Event Admin' : u.role.toUpperCase()}
                                        </span>
                                        {isInactive && (
