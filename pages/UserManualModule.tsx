...
type DocGroup = 'general' | 'operations' | 'delegates' | 'administration';
...
const GROUPS: { id: DocGroup; label: string }[] = [
...
const MANUAL_SECTIONS: DocSection[] = [
  {
    id: 'login',
    number: '01',
    group: 'general',
  ...
  },
  {
    ...
    body: (
      <>
        <p className="text-sm text-gray-600 mb-4">The Reports Center generates Attendance Lists, Attendance Matrices, Financial Ledgers, Pledge Summaries, and Session Reports. Reports respect per-event field visibility settings.</p>
  ...
  },
  {
    ...
    number: '04',
    group: 'operations',
    title: 'Operations: Arrival & QR Code Verification',
    allowedRoles: ROLE_CHECKIN,
    body: (
      <>
  ...
  },
  {
    ...
    number: '06',
    group: 'operations',
    title: 'Active Session Verification (QR & Delegate ID)',
    allowedRoles: ROLE_CHECKIN,
    body: (
      <>
  ...
  },
  {
    ...
    number: '05',
    group: 'operations',
    title: 'Operations: New Delegate Registration',
    allowedRoles: ROLE_CHECKIN,
    body: (
      <>
  ...
  },
  {
    ...
    number: '6A',
    group: 'operations',
    title: 'Session Details: Alter Call Recording & Tracking',
    allowedRoles: ROLE_CHECKIN,
    body: (
      <>
        <p className="text-sm text-gray-600 mb-4">The <strong>Session Details</strong> page captures delegate responses to altar calls during each session. Four response types are tracked per delegate:</p>
        <div className="space-y-4">
  ...
  },
  {
    ...
    number: '07',
    group: 'operations',
    title: 'Financials: Offerings & Pledges',
  ...
  },
  {
    ...
    number: '8A',
    group: 'operations',
    title: 'Badge Management & E-Badge Distribution',
    allowedRoles: ROLE_CHECKIN,
    body: (
      <>
        ...
        <div className="space-y-4">
          ...
          <div className="p-5 border rounded-2xl bg-gray-50">
            ...
            <p className="text-xs text-gray-600">Delegates receive the badge image via WhatsApp or Telegram, save it to their phone gallery, and display it full-screen at check-in points. The QR code renders clearly on mobile screens for reliable scanning at arm's length. The badge includes the delegate's name, district, and unique QR code for verification.</p>
          </div>
          <div className="p-5 border rounded-2xl bg-gray-50">
  ...
  },
  {
    ...
    number: '9A',
    group: 'delegates',
    title: 'Master List: Per-District Pagination & Export',
    allowedRoles: ROLE_BADGES,
    body: (
      <>
        <p className="text-sm text-gray-600 mb-4">The <strong>Master List</strong> now operates in dual mode for efficient handling of thousands of delegates:</p>
        <div className="space-y-4 text-xs font-bold text-gray-700">
          <div className="p-4 border rounded-xl bg-gray-50">
  ...
  },
  {
    ...
    title: 'Admin: Events Catalog & Configuration',
      ...
      <>
        ...
        <div className="space-y-4 text-xs font-bold text-gray-700">
          ...
          <div className="p-4 border rounded-xl bg-gray-50">
            ...
            Admins can enable per-event single-row delegate deletion in the Master List, and define which fields are required per delegate type on the New Delegate form (including optional EMS payment capture).
          </div>
          <div className="p-4 border rounded-xl bg-gray-50">
  ...
  },
  {
    ...
    number: '10',
    group: 'administration',
    title: 'System Setup & Chapters Registry',
    allowedRoles: ROLE_ADMIN,
    body: (
      <>
        ...
        <div className="space-y-4 text-xs font-bold text-gray-700">
          ...
          <div className="p-4 border rounded-xl bg-gray-50">
            <span className="text-blue-600 block mb-1">DELEGATE TYPE → DISTRICT ROUTING</span>
            Route certain delegate types to a fixed district (e.g., Free Guest and National Guest → Guest; International → International). These routes drive registration, Quick Registration, and RLS automatically.
          </div>
          <div className="p-4 border rounded-xl bg-gray-50">
  ...
  },
  {
    ...
    body: (
      <>
        <p className="text-sm text-gray-600 mb-4">System accounts are assigned a role by an Administrator in <strong>User Management</strong>. Each role controls which modules appear in the sidebar and what the account may read or write.</p>
        <div className="space-y-4 text-xs font-bold text-gray-700">
          <div className="p-4 border rounded-xl bg-gray-50">
...
/* ------------------------- VOLUNTEER TRAINING GUIDE SECTIONS ------------------------- */

const TRAINING_SECTIONS: DocSection[] = [
  {
    id: 'track-registrar',
    number: 'A',
    group: 'operations',
    title: "The Registrar's Scenarios",
    allowedRoles: ROLE_CHECKIN,
    body: (
      <div className="space-y-6">
  ...
  },
  {
    ...
    number: 'B',
    group: 'operations',
    title: "The Finance Officer's Track",
    allowedRoles: ROLE_FINANCE,
    body: (
      <div className="space-y-4">
  ...
  },
  {
    ...
    number: 'C',
    group: 'operations',
    title: "The Badge Printing Operator's Track",
    allowedRoles: ROLE_BADGES,
    body: (
      <div className="space-y-6">
        <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100">
...
const UserManualModule = () => {
  ...
  const visibleManual = useMemo(() => MANUAL_SECTIONS.filter(s => s.allowedRoles.includes(role)), [role]);
  const visibleTraining = useMemo(() => TRAINING_SECTIONS.filter(s => s.allowedRoles.includes(role)), [role]);
  const visible = activeTab === 'manual' ? visibleManual : visibleTraining;

  const visibleCount = activeTab === 'manual'
    ? MANUAL_SECTIONS.filter(s => s.allowedRoles.includes(role)).length
    : TRAINING_SECTIONS.filter(s => s.allowedRoles.includes(role)).length;
  ...
  const handleExport = () => {
    if (manualRef.current) {
      ...
      exportToPDF(manualRef.current, fileName, 'portrait', undefined, 'document');
    }
  };

  const renderSections = (activeList: DocSection[]) => (
    GROUPS.map(g => {
      const secs = activeList.filter(s => s.group === g.id);
      if (!secs.length) return null;
      return (
        <div key={g.id} className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            <span className="bg-slate-900 text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest">{g.label}</span>
          ...
          {secs.map(s => (
            <section key={s.id} id={s.id} className="mb-12">
              <h2 className="text-xl font-black text-blue-900 uppercase mb-4 flex items-center gap-3 border-b pb-2">
                ...
                <span>{s.title}</span>
                <span className="ml-auto text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 border border-slate-200">{GROUP_LABEL[s.group]}</span>
              </h2>
              <div>{s.body}</div>
            </section>
          ))}
  ...
  const renderToc = (activeList: DocSection[]) => (
    GROUPS.map(g => {
      const secs = activeList.filter(s => s.group === g.id);
      if (!secs.length) return null;
      return (
        <div key={g.id} className="mb-4">
          <div className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1.5 px-2">{g.label}</div>
          <div className="space-y-0.5">
            {secs.map(s => (
              <button
                key={s.id}
                onClick={() => scrollTo(s.id)}
                className="w-full text-left px-2.5 py-1.5 rounded-lg text-[11px] font-bold text-gray-600 hover:bg-blue-50 hover:text-blue-800 transition-colors"
              >
                <span className="text-blue-700 font-mono mr-1.5">{s.number}</span>{s.title}
              </button>
            ))}
          </div>
        </div>
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <div className="bg-white p-6 rounded-2xl shadow-sm border no-print space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2 px-1">
          ...
          <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-100 text-[9px] font-black uppercase tracking-widest">{getRoleLabel(role)}</span>
          <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">&middot; {visibleCount} section{visibleCount === 1 ? '' : 's'} available</span>
        </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {visible.map(s => (
              <button
                key={s.id}
                onClick={() => scrollTo(s.id)}
                className="shrink-0 px-3 py-1.5 rounded-full bg-gray-100 text-[10px] font-black uppercase tracking-wider text-gray-600 hover:bg-blue-50"
              >
                {s.number} {s.title}
              </button>
            ))}
      ...
      <div className="flex gap-6 items-start">
        <aside className="hidden lg:block w-72 shrink-0 no-print">
          <nav className="sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto bg-white rounded-2xl shadow-sm border p-5">
            <h3 className="text-xs font-black uppercase tracking-widest text-blue-900 mb-1">Contents</h3>
            <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-4">Grouped by sidebar section</p>
            {renderToc(visible)}
          </nav>
        </aside>
        ...
        <div ref={manualRef} className="flex-1 min-w-0 bg-white p-10 md:p-14 rounded-[2.5rem] shadow-sm border min-h-screen text-slate-800 leading-relaxed">
          ...
          {activeTab === 'manual' ? (
            renderSections(visibleManual)
          ) : (
            renderSections(visibleTraining)
          )}
