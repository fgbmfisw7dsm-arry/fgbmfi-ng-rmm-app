-- ============================================================
-- IMPORT ROW-GUARD — blank/numeric/summary rows can never become delegates
-- ============================================================
-- Root cause (NC3 import, Aug 2026): trailering summary blocks in manual-reg
-- CSVs (`Zone Summary`, `ZONE,Adults,Teens,Children,...`, `UPZ I,78,2,5,...`,
-- `GRAND TOTAL,...`, `Notes:`) passed the client sanitizer and were inserted
-- as delegates with numeric or blank name fields. Those rows surface FIRST in
-- the Master List because digits sort before letters in last_name ordering.
--
-- This migration hardens import_delegates_batch_merge (the DB backstop) so
-- that any incoming item with blank or purely-numeric first/last names is
-- counted as `skipped` and never INSERTED/UPDATed. Combined with the client
-- gate in ImportModule.mappedCsvData + supabaseService.importDelegates, this
-- provides defense in depth: even if a future client path forgets to validate,
-- the database refuses junk rows.
--
-- Builds on the Sprint 27 version (no-contact dedup + transposed pairs + the
-- admin/event_admin privilege gate). Idempotent: safe to re-run.
-- ============================================================
