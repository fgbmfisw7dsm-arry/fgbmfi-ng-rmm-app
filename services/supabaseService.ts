...
export const db = {
    ...
    getSettings: async (): Promise<SystemSettings> => {
        const defaultData = { titles: ['Mr', 'Mrs', 'Ms', 'Chief', 'Dr', 'Prof', 'Engr', 'Elder'], districts: ['North Central 1', 'North Central 2', 'North Central 3', 'North Central 4', 'North Central 5', 'North East 1', 'North East 2', 'North West 1', 'North West 2', 'North West 3', 'South East 1', 'South East 2', 'South East 3', 'South South 1', 'South South 2', 'South South 3', 'South South 4', 'South West 1', 'South West 2', 'South West 3', 'South West 4', 'South West 5', 'South West 6', 'South West 7', 'SOUTH WEST 8', 'National/External'], ranks: [], offices: [], regions: [], delegate_types: ['Member', 'National Guest', 'Free Guest', 'Dependant-Adult', 'Dependant-Teen', 'Dependant-Children', 'International'] };
    ...
    fetchAllDelegatesForExport: async (eventId: string, district?: string, search?: string, source?: 'portal' | 'manual'): Promise<Delegate[]> => {
        ...
        const pageSize = 500;
        while (true) {
            const paginated = await db.getPaginatedDelegates(page, pageSize, search || undefined, district || undefined, undefined, eventId, source);
    ...
    repairExternalId: async (delegateId: string): Promise<string | null> => {
        const { data: delegate } = await supabase.from('delegates').select('external_id').eq('delegate_id', delegateId).maybeSingle();
        if (!delegate) return null;
        if (delegate.external_id && delegate.external_id.startsWith('CON26')) return delegate.external_id;

        const regId = generateRegId();
        await supabase.from('delegates').update({ external_id: regId }).eq('delegate_id', delegateId);
        return regId;
    },
    ...
    getSourceDistribution: async (eventId: string): Promise<{ total: number; counts: Record<string, number> }> => {
        if (!eventId) return { total: 0, counts: {} };
    ...
    reclassifyDelegateSource: async (eventId: string, mode: 'portal' | 'manual', delegateIds?: string[]): Promise<number> => {
        ...
        const { data, error } = await q.select('delegate_id');
        if (error) throw error;
        const count = data?.length || 0;
        recordAuditLog(eventId, 'delegate_source_reclassify', `Reclassified ${count} delegates as ${mode}`, null, 'event', eventId, { mode, count });
        return count;
    },
};