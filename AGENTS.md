...
- **Cleanup for already-imported junk (`supabase_migration_nc3_junk_rows_cleanup.sql`):** manual-run script — resolves the NC3 event by name, backs up candidates to `_nc3_junk_backup`, `SELECT`s them for review, then deletes dependents (`badge_print_logs`/`session_responses`/`checkins`) then `delegates` scoped to that event (blank/numeric names, header-word names, `>`-laced names). Restore available via `INSERT INTO delegates SELECT * FROM _nc3_junk_backup`.
