...
| `system_settings` | Global config (single row) | id, titles (TEXT[]), districts (TEXT[]), ranks (TEXT[]), offices (TEXT[]), regions (TEXT[]), delegate_types (TEXT[]) | Authenticated |
...
- **Text sizing (Canvas, 6-up tier):** Name 12px bold (auto-fit ≥8px), Labels 7px bold, Values 8px bold, Delegate Type 12→6px white auto-fit; **ID value renders at −1pt (Log value 7px) with a width-fit cap** so long external_ids can never collide with the QR edge. **Canvas-local zone offset (`z` object over `V2_ZONES`):** because the 12px canvas name is ~2 lines taller than the PDF's 9.5pt name, the canvas name block + details band sit 2 lines lower (nameTop 0.542/nameBottom 0.659, band 0.670–0.877, field lineGap 14) to clear the design's baked-in "Theme" line — PDF layouts are unaffected.
...
- **NC2 chapter (Sep 2026):** the 14-09-26 `NC2 MANUAL EARLY BIRD…csv` (`S/N, FULL NAME, PHONE NUMBER, EMAIL, CHAPTER, ZONE`) reported "Chapter not extracted" — verified the current `splitCsvRecords` build maps `CHAPTER` (idx 4) → Chapter on the mapped output (`p[5]`), with 158/206 rows carrying a chapter (the rest are genuinely blank in the source file). No NC2-specific code defect — the NC1 quote-aware split + phone-alias fix covers this header shape. **General hardening added:** `chapter name`/`chaptername`/`chap`/`chapter of fgbmfi` now alias to `Chapter` (and the Repair Imported Names flow shares the same `KNOWN_FIELDS`), so ANY future district file with a variant Chapter header spelling is detected. All 19 district CSVs in the 14-09-26 batch map their `CHAPTER` column.
...
## 45. External/Guest CSV Email Backfill + Contact-less Merge Chaining (v1.47)

- **Email from `RegisteredBy` (frontend):** the Bulk Delegate Import mapper now fills `Email` from a `RegisteredBy`/`Registered By` column **only when the `Email` column is blank** (`ImportModule.tsx` `mappedCsvData`). The explicit `Email` column always wins. `RegisteredBy` is deliberately NOT added to `KNOWN_FIELDS` — `firstIdx('Email')` returns the *first* email-mapped column, so mapping it there could shadow a populated explicit `Email` column; it is a dedicated fallback that intentionally ignores `columnMap` (unknown headers are recorded as `false` there). All 16 rows of `External Delegates Reg-15-9-26.csv` verified mapping (e.g. `Douglas Uzochukwu Douglas → fnyuzochukwu@gmail.com`).
- **Root cause (duplication, Sep 2026):** `import_delegates_batch_merge` branched its identity lookup on the **incoming** row's contact data — phone → email → names-only contact-less. Once the CSV carried emails, a re-import over the 16 pre-existing contact-less `National/External | Guest` rows took the email branch (which requires an existing **email-bearing** row), found nothing, and **INSERTed a duplicate** per person (32 rows, each "doubled"). The names-only fallback only ever ran when the incoming row had no email.
- **Fix (`supabase_migration_v1.47_external_guest_import_fix.sql`, `supabase_schema.sql` reconciled):** the merge lookup is now **chained** (phone → email → names-only contact-less fallback), so a contact-bearing incoming row merges into a contact-less existing row (same event + `title_key`/`name_first_key`/`name_last_key` identity) and gap-fills its email **instead of inserting**. Prevents the multiplication for any future contact-less re-import. **Part 2** removes the 16 duplicate contact-less twins: backed up to `external_delegates_dupe_backup_20260915` (created via **`CREATE TABLE AS SELECT`** — a `LIKE INCLUDING ALL`+INSERT backup is rejected by PG15 with `428C9 cannot insert a non-DEFAULT value into generated column "name_display"`), deleted **only** when the row is contact-less (`phone_normalized`/`email` NULL), shares full identity with an email-bearing twin in the same event's `National/External` district, and has **zero** `checkins`/`session_responses`/`badge_print_logs`. Preview SELECT is included in the file. Idempotent — a second run is a no-op.
- **Operational (done once, no re-import needed):** run the whole migration after the email-tagging frontend is deployed. The 16 surviving rows already carry their emails; every later re-import is a true no-op (email lookup finds the row).

## 46. District Label Rename: 'National/External' → 'International/External' (v1.48)

- **Context (Sep 2026):** the guest district was renamed in System Setup (`system_settings.districts` now lists `International/External`), yet the Master List continued to show the old label. Root cause: Master List **"All Official Districts" section headers are rendered from the STORED `delegates.district` values** (`getDistrictsWithDelegates` + section per stored value) — not from `system_settings`. Renaming the setting only changed the dropdown; the 16 existing guest rows (and the app's write constants + RLS) still used `National/External`. This is stale **data** + stale **constants**, not a display hard-code in MasterListModule.
- **Frontend updates:** `NewDelegatePage.tsx` `FREE_GUEST_DISTRICT`, `registerDelegate` payload force-set and the empty-settings district seed (`supabaseService.ts`), and `ImportModule.tsx` `resolveGuestFields` GUE mapping all now write `International/External`.
...
## 49. QR Check-In Reconciliation — portal/3rd-party badge payloads (v1.51)

- **Problem solved (SW7, Sep 2026):** scanning a badge **printed from the external FGBMFI registration portal** (data-rich QR payload: portal RegId, name, short-code district `SW7`, chapter, phone, email) for a delegate already in the EMS (district `South West 7`) did NOT check them in. `checkInByCode`'s 3 passes (qr_hash / external_id / delegate_id) found no EMS identifier (portal RegId ≠ EMS `CON26` `external_id`, no qr_hash), so it fell through to the **Register Delegate** modal prefilled with `district='SW7'`. `registerDelegateFromQR`'s dedup was raw-phone equality (`.eq('phone', raw)` — portal `234803…`/`803…` vs normalized `0803…`), did no email/name-key fallback, matched `external_id` only when `CON26`-prefixed, and wrote the district verbatim → **duplicate person + phantom `SW7` district** (Master List sections render from stored `delegates.district` values).
- **Shared matcher (`matchDelegateByIdentity`, `supabaseService.ts`):** ladder of normalized passes — `normalizePhone` → email (`ilike`) → family-aware canonical name key (`name_key` column, fallback `ilike` first/last). Accepts **only a single strong candidate** (family-key disambiguation; multiple distinct rows = ambiguous → null). Event-scoped, defensive try/catch on optional Sprint-21/§37 columns (`phone_normalized`, `name_key`).
- **`checkInByCode` Pass 4:** after the 3 identifier passes, a payload with name data is resolved via `matchDelegateByIdentity` → direct `checkInDelegate` ("Verified"/"Already Checked-in"). Registration modal only appears for genuinely unknown pre-badged delegates. `lookupId` now also accepts `reg_id`/`RegId` keys.
- **`registerDelegateFromQR` hardened:** `external_id` pre-check accepts any stored id (any prefix, not just `CON26`); identity dedup via `matchDelegateByIdentity` (returns existing row instead of inserting); district canonicalized via new **`canonicalizeDistrict`** (routed types resolve `getConfiguredTypeDistrict` first, then `resolveDistrictShortCode`, then **validated against `system_settings.districts`** — unknown districts throw instead of creating phantom buckets); insert catches `23505` and re-resolves to the survivor.
- **CheckIn UI (`CheckInPage.tsx`):** the Register Delegate modal prefill resolves the scanned district via `resolveDistrictShortCode`; the District `<select>` turns amber with a warning when the value isn't an official district; `handleQuickRegister` blocks submit unless the district is official.
...
## 52. Single-RTT Check-In RPC (v1.54)

- **Purpose:** collapse the QR-scan hot path (previously ~5–12 sequential HTTPS round-trips/scan: guard + 3 identifier passes + §49 fuzzy + `checkInDelegate` writes) into **one** server-side call `check_in_by_code(...)`. Shipped with `USE_CHECKIN_RPC = true`; rollback = `USE_CHECKIN_RPC=false` (or DROP FUNCTION) — the client degrades to the classic TS path automatically.
- **Migration (`supabase_migration_checkin_rpc.sql`, deploy FIRST):** `SECURITY INVOKER` (caller's RLS governs every read/write — identical privilege surface to today's client queries; anon/PUBLIC EXECUTE revoked, `authenticated`+`service_role` only). Single transaction; returns `{ok, locked, already_checked_in, needs_parse, message, delegate{...7 snapshot fields}}`.
- **Conservative resolution — wrong auto-check-in is impossible:** identifier probes mirror client Pass 1–3 (event-scoped `qr_hash`/`external_id`/`delegate_id::text`, first-row like `maybeSingle`). Payload token probes (id-token `[A-Z0-9]{20,}`, phone via `normalize_phone_sql` on `phone_normalized`, email on `lower(email)`) resolve **only on an EXACTLY-ONE row match**; zero **or multiple** rows (shared family phone, same-name pairs) → `needs_parse` → the existing TS path (family-aware §37/§49 disambiguation, `needsRegistration`, district canonicalization) runs unchanged. No name/district parsing in SQL and no new identity logic — reuses `qr_hash`, `external_id`, `phone_normalized`, `normalize_phone_sql`, §53 email indexes.
...
- **Phone ISD picker:** `services/dialCodes.ts` exports `COUNTRY_DIAL_CODES` (full ~200-country ISO list, code-first labels `+234 · Nigeria`, Nigeria pinned top/default `+234`) consumed by the searchable combobox `components/CountryDialSelect.tsx` (typeahead by country name OR code, keyboard + ARIA, triggers a `+234 · Nigeria`-style button). The form composes `dialCode + localNumber` before submit; the existing `normalizePhone` canonicalizes (`+234803… → 08…`), so dedup keys (`phone_normalized`), badges, check-in and reports are byte-identical to pre-v1.55. Phone placeholder is the 10-digit local format (`803...` — NOT `080...`, which would double the leading zero with `+234`). **Mobile/width hardening:** the phone row uses `flex-wrap` and the number input `flex-1 min-w-[14ch]` so it always displays ≥ ~11 digits even when the combobox wraps to its own line on narrow/laptop 3-col layouts; the combobox selects via pointer events (`onPointerDown` + idempotent guard, doc `pointerdown` outside-close), uses a `text-base` (16px) search input with `autoCorrect/autoCapitalize/spellCheck` off (prevents iOS focus-zoom + autocorrect mangling country names), `touch-manipulation`, and wraps long country labels. Applied to the New Delegate form only.
...
## 54. Role-Scoped, Segment-Grouped System Documentation (v1.56)

- **Problem solved:** the `/help` page (`UserManualModule.tsx`) rendered **every** Operations-Manual section and every Training-Guide track to **every** role — including docs for modules the user cannot open (district registrars reading Events Catalog, Finance users reading Badge Printing). No segmentation, no role gating, one monolithic scroll.
- **Rebuild (`pages/UserManualModule.tsx`):** content moved from inline JSX into two data arrays — `MANUAL_SECTIONS` (15 sections) and `TRAINING_SECTIONS` (4: Track A / Track B / Track C / Troubleshooting). Each `DocSection = { id, number, group ('general'|'operations'|'delegates'|'administration'), title, allowedRoles: UserRole[], body }`. Content is byte-identical to the pre-existing copy (pure reorganization, no authoring). Group = the sidebar menu section where the module lives.
- **Role scoping (source of truth = App.tsx route-guard role lists):** section visibility is `s.allowedRoles.includes(user.role)` via shared role groups — `ROLE_CHECKIN` (admin tiers + registrar tiers + executive_admin + event_admin, mirrors `ADMIN_REGISTRAR_AND_EVENT_ADMIN`), `ROLE_FINANCE` (admins + finance + event_admin, mirrors `ADMIN_FINANCE_AND_EVENT_ADMIN`), `ROLE_BADGES` (admins + event_admin, mirrors `ADMIN_AND_EVENT_ADMIN`), `ROLE_ADMIN` (4 admin tiers, mirrors `ALL_ADMIN_ROLES`), `ALL_ROLES`. Net: registrar sees General + Check-In/New Delegate/Session/Alter-Call/E-Badge docs; Finance sees General + Financials docs; Event Admin sees everything but the four pure-admin Administration sections; Executive Admin sees registrar ops + financial *view* but not Track B recording or badge/admin docs.
- **Segment grouping + TOC:** document renders sections grouped under sidebar-section headers (General → Operations → Delegates → Administration) each with a nav badge; a sticky left **Contents** panel (desktop) + horizontal chip rail (mobile) lists the role-visible sections per group and scrolls via `scrollIntoView` (button-based, NOT `<a href="#…">` — avoids HashRouter hash collisions). `activeTab: 'manual' | 'training'` tabs preserved.
- **Deep link:** `App.tsx` help route widened to `<Route path="/help/:section?" …>`; `useParams().section` (e.g. `#/help/financials`, `#/help/track-badge`) scrolls to the section on mount when the role can see it (silent no-op otherwise).
- **Export:** "Export Active View (PDF)" unchanged (`exportToPDF(…,'document')`, filename per tab) — naturally role-scoped because the cloned `manualRef` DOM only contains visible sections.
- **Non-disruption:** single sidebar entry ("User Manual") unchanged; no `types.ts`/`supabaseService.ts`/DB changes; `vite build` passes (4 pre-existing non-blocking `tsc` warnings unchanged).
- **v1.56-fix — blank first-page body in the Volunteer Training Guide PDF export:** html2pdf 0.10.1's `css` pagebreak mode reads `break-inside: avoid` and inserts near-full-page white padding divs before any avoid element that straddles a page boundary. The Training Guide's tall Track A section (right after the hero) straddled the page-1 boundary, producing a ~1700px blank band under the header and cascading pages (verified empirically: a straddling document rendered page 3 at 11% content / max gap 1737px). **Part 1:** `exportToPDF` passes `pagebreak: { mode: [] }` in **document mode** (manual/training exports — reports keep css mode which relies on `tr { page-break-inside: avoid }`), and `UserManualModule` section wrappers no longer carry `break-inside-avoid` (no on-screen effect; screen print semantics already covered). Verified via headless html2pdf canvas analysis: max blank gap drops 1737px → ≤249px, no near-blank pages, content stays in flow. **Part 2 (Scenario 5 cut mid-card):** disabling pagebreak entirely made html2pdf slice every card at the fixed A4 boundary — the blue "New Visitor" card spanned y[2084,2456] with page boundary y=2169 cutting it in half (unreadable). Refined to `pagebreak: { mode: [], avoid: '.rounded-xl, .rounded-2xl, .rounded-3xl' }` in document mode, so bounded cards are pushed wholesale onto the next page (`avoid` only triggers for elements ≤1 page tall → no giant gaps). Verified: `blueCardCuts: []`, page content 82–89%, max gap ≤252px.

## 55. Pledge Event Session + Financial Matrix Redeemed (v1.58)

- **Problem solved (two bugs + one enhancement):** (1) The Financials **New Pledges** table showed Redeemed = 0.0 even after partial redemption — `pledges.amount_redeemed` was never maintained (the client records redemptions as `financial_entries` rows via `db.addFinancialEntry`, never calling the legacy `update_pledge_redemption` RPC), while **ReportsPage already computed redeemed from entries** (single source of truth). (2) The **Financial Matrix** and **Sessions Summary** bucket financial entries by `session_id`, but Redemptions were recorded with **no session** → PLEDGE_REDEMPTION amounts were excluded from every session column AND from the row Total (money vanished). (3) Enhancement requested by finance: record the **Event Session where the pledge was made**.
- **Schema (`supabase_migration_v1.58_pledge_session_matrix.sql`, idempotent):** adds nullable `pledges.session_id UUID REFERENCES sessions(session_id) ON DELETE SET NULL` (a pledge commitment survives session deletion; its redemptions then fall into the Master bucket) + `idx_pledges_event_session(event_id, session_id)`. No RPC changes — `get_report_aggregates` selects `*` from pledges so the column flows through automatically.
- **types.ts additive exception:** `Pledge.session_id?: string` — mirrors the `pledge_name` precedent.
- **`FinancialsPage.tsx`:** new `redeemedByPledge` map computed from the already-fetched `entries` (type `PLEDGE_REDEMPTION`, keyed by `pledge_id`) + `redeemedOf(p)` helper — replaces **every** `p.amount_redeemed` read (Redeemed/Balance columns, grand totals, redemption-search remaining balance, `handleSelectPledge` default amount + `Bal:` display, pledge CSV/PDF exports). **New Pledge form** gains an Event Session dropdown (`pForm.session_id`, blank = "Full Event (Master)"). **Redemption form** gains an Event Session dropdown pre-filled from the selected pledge's `session_id` (overridable); `submitRedemption` sends `session_id: rForm.session_id || undefined` — this is what makes redeemed pledges appear in the matrix. Pledge and Redemption CSV/PDF exports gained a **Session** column.
