...
- **Camera selection robustness (OBS Virtual Camera):** devices are ranked `cameraRank()` — USB/external webcams first, built-in next, virtual devices (OBS/VCam/simulators) last. Streams are opened with `deviceId: { exact }` (never `ideal`). `startBarcodeDetector` iterates candidates in rank order and verifies the live track via `getSettings().deviceId`; any virtual result from a physical request is rejected and the next candidate is tried. On total aliasing (every physical request returns OBS), the scanner shows a visible warning: "Showing OBS Virtual Camera — close or stop OBS to use the USB webcam." The active camera is displayed as a green pill in the scan area (`activeCameraLabel`) so the live source is always visible. Stale saved virtual `qr-camera-device-id` entries are purged at init.
