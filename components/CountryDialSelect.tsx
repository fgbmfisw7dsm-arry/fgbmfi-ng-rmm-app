...
const CountryDialSelect = ({ value, onChange }: CountryDialSelectProps) => {
    ...
    useEffect(() => {
        ...
        const li = listRef.current?.querySelector<HTMLElement>(`[data-idx="${highlight}"]`);
    ...
    return (
        <div ref={rootRef} className="relative shrink-0 min-w-0">
            <button
                ...
                onClick={() => { pickedRef.current = false; if (open) { setOpen(false); } else { setOpen(true); setQuery(''); setHighlight(0); } }}
                className={`flex items-center justify-between gap-2 p-4 border-2 rounded-2xl bg-gray-50 font-black text-xs whitespace-nowrap outline-none focus:bg-white active:scale-[0.98] transition-all touch-manipulation ${open ? 'border-blue-500 bg-white' : 'border-gray-50 focus:border-blue-500'}`}
            >
                ...
                <div className="absolute z-50 mt-2 w-72 max-w-[92vw] rounded-2xl bg-white border border-gray-200 shadow-2xl overflow-hidden">
                    <div className="p-3 border-b border-gray-100">
                        <input
                            ...
                            autoCapitalize="none"
                            spellCheck={false}
                            className="w-full p-3 border-2 border-gray-100 rounded-xl bg-gray-50 font-bold text-base outline-none focus:border-blue-500"
                        />
                    </div>
                    <ul ref={listRef} role="listbox" className="max-h-64 overflow-y-auto">
                        ...
                        {filtered.map((c, i) => (
                            <li
                                ...
                                onPointerDown={(e) => { e.preventDefault(); pick(c.code); }}
                                onClick={() => pick(c.code)}
                                onMouseEnter={() => setHighlight(i)}
                                className={`px-4 py-2.5 cursor-pointer font-bold text-xs uppercase tracking-wide break-words touch-manipulation ${i === highlight ? 'bg-blue-50 text-blue-900' : c.code === value ? 'bg-teal-50 text-gray-700' : 'text-gray-600'}`}
                            >
                                {c.label}
                            </li>
                        ))}
